	...
	tests := []struct {
		...
		},
		{
			...
			checkFunc: func(got, want Server, gotErr, wantErr error) error {
				...
				gotCipherSuites := got.(*server).srv.TLSConfig.CipherSuites
				wantCipherSuites := want.(*server).srv.TLSConfig.CipherSuites
				sort.Slice(gotCipherSuites, func(i, j int) bool {
					return gotCipherSuites[i] < gotCipherSuites[j]
				})
				sort.Slice(wantCipherSuites, func(i, j int) bool {
					return wantCipherSuites[i] < wantCipherSuites[j]
				})
				if !reflect.DeepEqual(gotCipherSuites, wantCipherSuites) {
					return fmt.Errorf("CipherSuites not equals\tgot: %v\twant: %v", gotCipherSuites, wantCipherSuites)
				}
				return nil
			},
		},
		{
			...
			want: &server{
				srv: &http.Server{
					Addr: fmt.Sprintf(":%d", 9999),
					TLSConfig: &tls.Config{
						CipherSuites: func() (cipherSuites []uint16) {
